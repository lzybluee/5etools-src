				...
				Renderer.get().withPlugin({
					...
					fn: () => {
						const trSubclassFeature = veT`<tr class="cls-main__sc-feature" data-subclass-id="${UrlUtil.getStateKeySubclass(sc)}"></tr>`
							.vee.html(`<td colspan="6">${Renderer.get().withDepthTracker(depthArr, ({renderer}) => renderer.render(Renderer.class.getDisplayNamedSubclassFeatureEntry(toRender, {styleHint})), {additionalProps: ["isReprinted"], additionalPropsInherited: ["_isStandardSource", "isClassFeatureVariant"]})}</td>`)
							.vee.appendTo(wrpContent);
					},
				});
